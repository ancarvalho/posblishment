...
class ProductStore extends StreamStore<Failure, int> {
  ...
  Future<void> createProduct(NewProduct product) async {
    if(!isLoading) {
      return executeEither(() => DartzEitherAdapter.adapter(_createProduct(product)) );
    }
    ManagementError(StackTrace.current,"AdministrationModule-createProduct", "", "Currently Executing Action");
  }

  Future<void> updateProduct(UpdateProductModel product) async {
    if(!isLoading) {
      return executeEither(() => DartzEitherAdapter.adapter(_updateProduct(product)));
    }
    ManagementError(StackTrace.current,"AdministrationModule-updateProduct", "", "Currently Executing Action");
  } 
}
